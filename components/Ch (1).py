...
spec1 = importlib.util.spec_from_file_location(
    "group1", "GROUP35(41120221433,41120221877,41120220847,41120211626).py"
)
group1_module = importlib.util.module_from_spec(spec1)
spec1.loader.exec_module(group1_module)
group1 = group1_module.group1

spec2 = importlib.util.spec_from_file_location("group2", "group2.py")
group2_module = importlib.util.module_from_spec(spec2)
spec2.loader.exec_module(group2_module)
group2 = group2_module.group2
...
class Bot:
    ...
    def simulateMove(self, board, move):
       ...
       final_pos = move[2][0] 
       simulated_board.move_piece(current_pos[0], current_pos[1], final_pos[0], final_pos[1])
       if final_pos not in simulated_board.getAdjacentSquares(current_pos[0], current_pos[1]):
         captured_x = current_pos[0] + (final_pos[0] - current_pos[0]) // 2
         captured_y = current_pos[1] + (final_pos[1] - current_pos[1]) // 2
         simulated_board.remove_piece(captured_x, captured_y)

       return simulated_board
    def simulateOpponentMove(self, board):
       simulated_board = deepcopy(board)
       opponent_moves = []
       for i in range(8):
           for j in range(8):
               squarePiece = simulated_board.getSquare(i, j).squarePiece
               if (
                squarePiece is not None
                and squarePiece.color == self.opponent_color
                and simulated_board.get_valid_legal_moves(i, j) != []
               ):
                opponent_moves.append(
                       (i, j, simulated_board.get_valid_legal_moves(i, j))
                )
       ...
       for move in opponent_moves:
           temp_board = deepcopy(simulated_board)
           self.moveOnBoard(temp_board, (move[0], move[1]), move[2][0], continue_playing=False)
           move_score = self.evaluate(temp_board)
           if move_score > best_score:
               best_score = move_score
               best_move = move
       if best_move:
           self.moveOnBoard(
               simulated_board, (best_move[0], best_move[1]), best_move[2][0], continue_playing=False
           )
